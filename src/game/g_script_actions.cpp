...
qboolean G_ScriptAction_Wait( gentity_t *ent, char *params ) {
	...
	return (qboolean) ( ent->scriptStatus.scriptStackChangeTime + duration < level.time );
}
...
qboolean G_ScriptAction_Trigger( gentity_t *ent, char *params ) {
	...
	if ( trent ) {
		...
		return (qboolean)(( ( trent != ent ) || ( oldId == trent->scriptStatus.scriptId ) ));
	}
...
qboolean G_ScriptAction_PlayAnim( gentity_t *ent, char *params ) {
	...
	return (qboolean)( endtime <= level.time );
};
