...
int PC_ExpandDefine( source_t *source, token_t *deftoken, define_t *define,
	...
	for ( dt = define->tokens; dt; dt = dt->next )
	{
		...
		{
			//if stringizing __operator
			if ( dt->string[0] == '#' && dt->string[1] == '\0' ) {
				//the stringizing __operator must be followed by a define parameter
				if ( dt->next ) {
					parmnum = PC_FindDefineParm( define, dt->next->string );
				} else { parmnum = -1;}
				//
				if ( parmnum >= 0 ) {
					//step over the stringizing __operator
					dt = dt->next;
				...
				{
					SourceWarning( source, "stringizing __operator without define parameter" );
					continue;
				} //end if
			} //end if
			else
	  ...
	  //check for the merging __operator
	for ( t = first; t; )
	{
		if ( t->next ) {
			//if the merging __operator
...
#define AllocOperator( op )								  \
	if ( numoperators >= MAX_OPERATORS ) {				  \
		SourceError( source, "out of __operator space\n" );	\
		error = 1;										\
		break;											\
	}													\
	else { \
...
int PC_EvaluateTokens( source_t *source, token_t *tokens, signed long int *intvalue,
		...
		case TT_PUNCTUATION:
		{
			...
			if ( !integer ) {
				if ( t->subtype == P_BIN_NOT || t->subtype == P_MOD ||
					 ...
					 t->subtype == P_BIN_XOR ) {
					SourceError( source, "illigal __operator %s on floating point operands\n", t->string );
					error = 1;
					break;
				}     //end if
			}     //end if
			...
			{
				if ( !lastwasvalue ) {
					SourceError( source, "__operator %s after __operator in #if/#elif", t->string );
					error = 1;
					break;
				}         //end if
				break;
			...
			default:
			{
				SourceError( source, "invalid __operator %s in #if/#elif", t->string );
				error = 1;
				break;
			}         //end default
			}     //end switch
	...
	if ( !error ) {
		if ( !lastwasvalue ) {
			SourceError( source, "trailing __operator in #if/#elif" );
			error = 1;
		} //end if
		else if ( parentheses ) {
	...
	while ( !error && firstoperator )
	{
		...
		{
			//if the current __operator is nested deeper in parentheses
			//than the next __operator
			if ( o->parentheses > o->next->parentheses ) {
				break;
			}
			//if the current and next __operator are nested equally deep in parentheses
			if ( o->parentheses == o->next->parentheses ) {
				//if the priority of the current __operator is equal or higher
				//than the priority of the next __operator
				if ( o->priority >= o->next->priority ) {
					break;
				}
			} //end if
			  //if the arity of the __operator isn't equal to 1
			if ( o->__operator != P_LOGIC_NOT
				 && o->__operator != P_BIN_NOT ) {
				v = v->next;
			}
...
#ifdef DEBUG_EVAL
		if ( integer ) {
			Log_Write( "__operator %s, value1 = %d", PunctuationFromNum( source->scriptstack, o->__operator ), v1->intvalue );
			if ( v2 ) {
		...
		{
			Log_Write( "__operator %s, value1 = %f", PunctuationFromNum( source->scriptstack, o->__operator ), v1->floatvalue );
			if ( v2 ) {
		...
		lastoperatortype = o->__operator;
		//if not an __operator with arity 1
		if ( o->__operator !=P_LOGIC_NOT
			 && o->__operator !=P_BIN_NOT ) {
			//remove the second value if not question mark __operator
			if ( o->__operator != P_QUESTIONMARK ) {v = v->next;}
			//
			if ( v->prev ) {
				v->prev->next = v->next;
