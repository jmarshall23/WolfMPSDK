...
gotnewcl:
	...
	*newcl = temp;
	...
	denied = (char *)gvm->ClientConnect(clientNum, qtrue, qfalse ); // firstTime = qtrue
	if ( denied ) {
		NET_OutOfBandPrint( NS_SERVER, from, "print\n%s\n", denied );
...
void SV_DropClient( client_t *drop, const char *reason ) {
	...
	gvm->ClientDisconnect(drop - svs.clients );
...
void SV_ClientEnterWorld( client_t *client, usercmd_t *cmd ) {
	...
	gvm->ClientBegin(client - svs.clients );
}
...
static void SV_VerifyPaks_f( client_t *cl ) {
	...
	if ( sv_pure->integer != 0 ) {
		...
		bGood = (qboolean)( FS_FileIsInPAK( FS_ShiftStr( SYS_DLLNAME_CGAME, -SYS_DLLNAME_CGAME_SHIFT ), &nChkSum1 ) == 1 );
		if ( bGood ) {
			bGood = (qboolean)( FS_FileIsInPAK( FS_ShiftStr( SYS_DLLNAME_UI, -SYS_DLLNAME_UI_SHIFT ), &nChkSum2 ) == 1 );
		}
...
static void SV_UpdateUserinfo_f( client_t *cl ) {
	...
	gvm->ClientUserinfoChanged(cl - svs.clients );
}
...
void SV_ExecuteClientCommand( client_t *cl, const char *s, qboolean clientOK ) {
	...
	if ( clientOK ) {
		...
		if ( !u->name && sv.state == SS_GAME ) {			
			gvm->ClientCommand(cl - svs.clients );
		}
	} else if ( !bProcessed )     {
...
void SV_ClientThink( client_t *cl, usercmd_t *cmd ) {
	...
	gvm->ClientThink(cl - svs.clients );
}
