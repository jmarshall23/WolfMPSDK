...
void R_SetFrameFog( void ) {
	...
	if ( glfogsettings[FOG_TARGET].finishTime && glfogsettings[FOG_TARGET].finishTime >= tr.refdef.time ) {
		...
		// transitioning like fog modes
			...
			glfogsettings[FOG_CURRENT].clearscreen  = (qboolean)( glfogsettings[FOG_TARGET].clearscreen || glfogsettings[FOG_LAST].clearscreen );
		}
	} else {
...
===============
*/
void R_Radix(int byte, int size, drawSurf_t* source, drawSurf_t* dest)
{
	int           count[256] = { 0 };
	int           index[256];
	int           i;
