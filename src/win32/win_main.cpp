...
void Sys_UnloadDll(void* dllHandle) {
	...
	if (!FreeLibrary((HINSTANCE)dllHandle)) {
...
Builds a platform-specific DLL name
=================
*/
...
Loads a DLL from various possible paths. Returns a handle to the loaded library.

- name: Name of the DLL (without extension)
- fqpath: Fully-qualified path buffer where the final load path
		  will be written (must be at least MAX_QPATH+1 in size).
=================
*/
void* Sys_LoadDll(const char* name, char* fqpath) {
	HINSTANCE libHandle;
	char* basepath;
	char* cdpath;    // unused here, but left if you still have a cvar
	char* gamedir;
	char* fn;
	...
	*fqpath = '\0';
	...
	cdpath = Cvar_VariableString("fs_cdpath");  // not used in this snippet
	gamedir = Cvar_VariableString("fs_game");

	// Attempt to load from "<basepath>/<gamedir>/filename"
	fn = FS_BuildOSPath(basepath, gamedir, filename);
	libHandle = LoadLibrary(fn);

	if (!libHandle) {
		// Fallback #1: "<basepath>/main/filename"
...
Wrapper to retrieve a function pointer from a loaded DLL
=================
*/
