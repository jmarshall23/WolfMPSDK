...
qboolean CG_EntOnFire( centity_t *cent ) {
	if ( cent->currentState.number == cg.snap->ps.clientNum ) {
		return  (qboolean)((   ( cg.snap->ps.onFireStart < cg.time ) &&
					( ( cg.snap->ps.onFireStart + 2000 ) > cg.time ) ));
	} else {
		return  (qboolean)((   ( cent->currentState.onFireStart < cg.time ) &&
...
#define ANIM_SPEEDMAX_HIGH  20
	...
	isLadderAnim = (qboolean)(lf->animation && ( lf->animation->flags & ANIMFL_LADDERANIM ));
...
static void CG_TrailItem( centity_t *cent, qhandle_t hModel ) {
	...
	if ( cent->currentState.clientNum == cg.snap->ps.clientNum ) {
		ducking = (qboolean)( cg.snap->ps.pm_flags & PMF_DUCKED );
	} else {
