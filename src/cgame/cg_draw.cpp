...
static void CG_ActivateLimboMenu( void ) {
	...
	if ( cg.snap->ps.pm_type == PM_INTERMISSION ) {
		...
		test = (qboolean)(cg.snap->ps.pm_flags & PMF_LIMBO || cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR || cg.warmup);
	}
...
static void CG_DrawSpectatorMessage( void ) {
	...
	CG_DrawStringExt( x, y, str, color, qtrue, qfalse, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 0 );
	y += TINYCHAR_HEIGHT;
