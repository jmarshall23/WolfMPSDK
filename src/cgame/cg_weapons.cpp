...
qboolean CG_MonsterUsingWeapon( centity_t *cent, int aiChar, int weaponNum ) {
	return (qboolean)(( cent->currentState.aiChar == aiChar ) && ( cent->currentState.weapon == weaponNum ));
}
...
// jpw
	...
	CG_WeaponIndex( curweap, &bank, &cycle );     // get bank/cycle of current weapon
	banked = (qboolean)CG_WeaponIndex( num, &newbank, &newcycle );      // get bank/cycle of requested weapon
