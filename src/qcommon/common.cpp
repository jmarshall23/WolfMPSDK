...
void *Z_TagMalloc( int size, int tag ) {
	...
	if ( extra > MINFRAGMENT ) {
		...
		newp = ( memblock_t * )( (byte *)base + size );
		newp->size = extra;
...
Goals:
  ...
  If not, we can choose to make either side the newp temp side and push future
...
The server calls this before shutting down or loading a newp map
=================
*/
...
void Com_SetRecommended() {
	...
	goodVideo = (qboolean)( cv && cv->integer );
	goodCPU = (qboolean)Sys_GetHighQualityCPU();
...
acos(*(float*) &i) == -1.#IND0
	...
	This should go in q_math but it is too late to add newp traps
	to game and ui
=====================
*/
